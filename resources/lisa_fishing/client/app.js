...
function showMSG(type, diff, keys) {
    ...
    if(type == "fishMessage1") {
    ...
    else if(type == "fishMessage3" && diff != undefined && keys != undefined) {
        document.getElementById("keysPuzzle").innerHTML = "";
        for (let index = 0; index < keys.length; index++) {
            document.getElementById("keysPuzzle").innerHTML += `<button id="game${index}" type="button" class="btn btn-light keybk unsolved">${keys[index]}</button>`;
        }
        ...
        //diff is number of keys to push.
        console.log('minigame app.js, diff: '+diff+', keys: '+keys);
    }
}
...
function setSolved(index) {
    ...
    /*
    console.log('index: '+index+', currentDiff: '+currentDiff);
    if(Number(index) == Number(currentDiff)-1) {
        console.log('solved complete ???');
    }
    */
}
